...
export function createCanvasLayers(container: HTMLElement, config: Required<CanvasOptions>): CanvasLayers {
  ...
  let transformLayer = container.querySelector(`.${CSS_CLASSES.TRANSFORM_LAYER}`) as HTMLElement;
  if (!transformLayer) {
  ...
  let contentLayer = transformLayer.querySelector(`.${CSS_CLASSES.CONTENT_LAYER}`) as HTMLElement;
  if (!contentLayer) {
...
function moveExistingContent(
  ...
  existingContent.forEach((child) => {
    if (child !== transformLayer && !child.classList.contains(CSS_CLASSES.TRANSFORM_LAYER)) {
