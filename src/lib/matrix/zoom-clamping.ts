...
export function clampZoomWithFeedback(scale: number, options: ZoomBoundaryOptions = {}): ZoomBoundaryResult {
  ...
  // Validate input
  if (typeof scale !== "number" || !Number.isFinite(scale) || Number.isNaN(scale)) {
  ...
  if (scale < config.minZoom) {
    ...
    result.message = `Minimum zoom level reached (${Math.round(config.minZoom * 100)}%)`;
    ...
    result.message = `Maximum zoom level reached (${Math.round(config.maxZoom * 100)}%)`;
