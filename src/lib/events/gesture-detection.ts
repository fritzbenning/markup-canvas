...
export function detectTrackpadGesture(event: WheelEvent): GestureInfo {
	...
	let trackpadScore = 0;
	if (criteria.smallDelta)
		trackpadScore += GESTURE_DETECTION_WEIGHTS.SMALL_DELTA;
	if (criteria.pixelMode) trackpadScore += GESTURE_DETECTION_WEIGHTS.PIXEL_MODE;
	if (criteria.fractionalDelta)
		trackpadScore += GESTURE_DETECTION_WEIGHTS.FRACTIONAL_DELTA;
	if (criteria.hasBothAxes)
		trackpadScore += GESTURE_DETECTION_WEIGHTS.BOTH_AXES;
	...
	result.confidence =
		Math.max(trackpadScore, mouseScore) / GESTURE_DETECTION_WEIGHTS.MAX_SCORE;
