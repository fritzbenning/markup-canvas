 ...
 */
export function limitZoomFactor(requestedZoomFactor: number): number | null {
  ...
  const limitedZoomFactor = Math.max(
    ZOOM_FACTOR_LIMIT.MIN_ZOOM_FACTOR_PER_FRAME,
    Math.min(requestedZoomFactor, ZOOM_FACTOR_LIMIT.MAX_ZOOM_FACTOR_PER_FRAME),
  );
