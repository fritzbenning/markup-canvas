/**
 * Adjusts `(x, y)` by subtracting `rulerSize` on both axes when a `.canvas-ruler` is present; otherwise uses the original coordinates.
 * Passes the result to `operation`.
 */
