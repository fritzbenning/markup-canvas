...
import type { Canvas, CanvasBounds, MarkupCanvasConfig } from "@/types/index";
import { DEFAULT_CONFIG } from "../../config/constants";
import { getEmptyBounds } from "./getEmptyBounds";
import { getVisibleBounds } from "./getVisibleBounds";

/**
 * Computes viewport size, content size, visible content rectangle, scaled sizes, and pan/zoom affordance flags
 * from the container layout and current transform.
 *
 * @param canvas - Canvas whose container and transform are measured.
 * @param config - Resolved configuration (content dimensions, ruler size).
 * @returns A {@link CanvasBounds} snapshot, or {@link getEmptyBounds} when measurement fails.
 */
export function getCanvasBounds(canvas: Canvas, config: Required<MarkupCanvasConfig>): CanvasBounds {
  try {
    ...
    const visibleArea = getVisibleBounds(canvasWidth, canvasHeight, contentWidth, contentHeight, transform);
